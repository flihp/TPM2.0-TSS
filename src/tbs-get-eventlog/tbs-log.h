/* SPDX-License-Identifier: BSD-2 */
#ifndef TBS_LOG_H
#define TBS_LOG_H

#include <windows.h>

#pragma pack(push,1)

typedef UINT32 TCG_PCRINDEX;
typedef UINT32 TCG_EVENTTYPE;
typedef UINT8 TCG_DIGEST[20];

/* Event structure for events from the 1.2 log format. */
typedef struct {
	TCG_PCRINDEX PCRIndex;
	TCG_EVENTTYPE EventType;
	TCG_DIGEST Digest;
	UINT32 EventSize;
	UINT8 Event[];
} TCG_PCR_EVENT;

/*
 * The following structures are combined to parse log events in the
 * TCG_PCR_EVENT2 structure
 */
typedef struct {
	UINT16 AlgorithmId;
	UINT8 Digest[];
} TCG_DIGEST2;

typedef struct {
	UINT32 EventSize;
	UINT8 Event[];
} TCG_EVENT2;

typedef struct {
	TCG_PCRINDEX PCRIndex;
	TCG_EVENTTYPE EventType;
	UINT32 DigestCount;
	TCG_DIGEST2 Digests[];
	/* TCG_EVENT2 comes next */
} TCG_EVENT_HEADER2;

/*
 * This is cosmetic: it's more natural for us to be passing around
 * structures with the same names used in the spec.
 */
typedef TCG_EVENT_HEADER2 TCG_PCR_EVENT2;
/*
 * Log event types. These are spread out over 2 specs:
 * "TCG EFI Protocol Specification For TPM Family 1.1 or 1.2" and
 * "TCG PC Client Specific Implementation Specification for Conventional BIOS"
 */
#define EV_PREBOOT_CERT            0x0
#define EV_POST_CODE               0x1
#define EV_UNUSED                  0x2
#define EV_NO_ACTION               0x3
#define EV_SEPARATOR               0x4
#define EV_ACTION                  0x5
#define EV_EVENT_TAG               0x6
#define EV_S_CRTM_CONTENTS         0x7
#define EV_S_CRTM_VERSION          0x8
#define EV_CPU_MICROCODE           0x9
#define EV_PLATFORM_CONFIG_FLAGS   0xa
#define EV_TABLE_OF_DEVICES        0xb
#define EV_COMPACT_HASH            0xc
#define EV_IPL                     0xd
#define EV_IPL_PARTITION_DATA      0xe
#define EV_NONHOST_CODE            0xf
#define EV_NONHOST_CONFIG          0x10
#define EV_NONHOST_INFO            0x11
#define EV_OMIT_BOOT_DEVICE_EVENTS 0x12
/*
 * TCG EFI Platform Specification For TPM Family 1.1 or 1.2
 */
#define EV_EFI_EVENT_BASE                0x80000000
#define EV_EFI_VARIABLE_DRIVER_CONFIG    EV_EFI_EVENT_BASE + 0x1
#define EV_EFI_VARIABLE_BOOT             EV_EFI_EVENT_BASE + 0x2
#define EV_EFI_BOOT_SERVICES_APPLICATION EV_EFI_EVENT_BASE + 0x3
#define EV_EFI_BOOT_SERVICES_DRIVER      EV_EFI_EVENT_BASE + 0x4
#define EV_EFI_RUNTIME_SERVICES_DRIVER   EV_EFI_EVENT_BASE + 0x5
#define EV_EFI_GPT_EVENT                 EV_EFI_EVENT_BASE + 0x6
#define EV_EFI_ACTION                    EV_EFI_EVENT_BASE + 0x7
#define EV_EFI_PLATFORM_FIRMWARE_BLOB    EV_EFI_EVENT_BASE + 0x8
#define EV_EFI_HANDOFF_TABLES            EV_EFI_EVENT_BASE + 0x9
#define EV_EFI_VARIABLE_AUTHORITY        EV_EFI_EVENT_BASE + 0xe0

/*
 * The following structures, when combined properly, can be used to parse
 * the TCG_EfiSpecIdEventStruct. This structure is defined in the TCG
 * UEFI protocol spec. It's easier to find in the documentation for the
 * Tbsi_Get_TCG_Log function:
 * https://docs.microsoft.com/en-us/windows/desktop/api/tbs/nf-tbs-tbsi_get_tcg_log
 */
typedef struct {
	UINT16 HashAlg;
	UINT16 DigestSize;
} SpecIdEventAlgSize;

/*
 * If the 'Signature' field is the '\0' terminated string "Spec ID Event03"
 * all Events will be TCG_PCR_EVENT2 structures. If not then they will be
 * TCG_PCR_EVENT structures.
 */
#define TPM2_LOG_SIG "Spec ID Event03"
#define TPM2_LOG_SIG_LENGTH sizeof (TPM2_LOG_SIG)
typedef struct {
	BYTE Signature[16];
	UINT32 PlatformClass;
	UINT8 SpecVersionMinor;
	UINT8 SpecVersionMajor;
	UINT8 SpecErrata;
	UINT8 UintNSize;
} SpecIdEventHeader;

typedef struct {
	UINT32 Count;
	SpecIdEventAlgSize DigestSizes[];
} SpecIdEventAlgs;

typedef struct {
	UINT8 Size;
	UINT8 VendorInfo[];
} SpecIdEventVendor;

#pragma pack(pop)

#endif