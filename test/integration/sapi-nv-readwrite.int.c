...
test_invoke (TSS2_SYS_CONTEXT *sys_ctx)
{
    ...
    return_if_error (rc, "nv_write_read_test for INDEX_LCP_OWN");
    rc = nv_write_read_test (sys_ctx, INDEX_LCP_SUP);
    return_if_error (rc, "nv_write_read_test for INDEX_LCP_SUP");
    rc = teardown_nv(sys_ctx, INDEX_LCP_OWN);
    return_if_error (rc, "teardown_nv for INDEX_LCP_OWN");
    rc = teardown_nv(sys_ctx, INDEX_LCP_SUP);
    return_if_error (rc, "teardown_nv for INDEX_LCP_SUP");
